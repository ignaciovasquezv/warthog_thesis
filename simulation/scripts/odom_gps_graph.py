...
# Lee los datos desde los archivos CSV
gps_data = pd.read_csv('/home/nacho/warthog_ws/src/package_nacho/data/gps_data_lidar2.csv').iloc[0:]
odometry_data = pd.read_csv('/home/nacho/warthog_ws/src/package_nacho/data/odometry_data_lidar2.csv').iloc[0:]
...
initial_odometry_x = rotated_odometry_x.iloc[0]
initial_odometry_y = rotated_odometry_y.iloc[0]

# Calcular las diferencias entre la posición inicial de GPS y las demás posiciones de GPS
diff_lat = gps_data['Latitude']
diff_lon = gps_data['Longitude']
...
plt.scatter(rotated_odometry_x, rotated_odometry_y, label='Odometría (IMU + Rueda)', color='blue')

# Trayectoria de GPS transformada a coordenadas relativas con la escala ajustada
plt.scatter(relative_gps_lat, relative_gps_lon, label='GPS (Relativo)', color='red')
